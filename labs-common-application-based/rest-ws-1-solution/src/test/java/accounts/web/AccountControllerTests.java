...
public class AccountControllerTests {
    ...
    public void testCreateAccount() {
        ...
        Assert.assertEquals("response.locationHeader", accountsUrl + '/' + ACCOUNT0_ID,
                            response.getHeaders().getLocation().toString());
        Assert.assertEquals("response.statusCode", HttpStatus.CREATED, response.getStatusCode());
        Assert.assertFalse("no response.body", response.hasBody());
    ...
    public void testGetBeneficiary() {
        ...
        final ResponseEntity<Beneficiary> beneficiaryResponse = controller.getBeneficiary(ACCOUNT0_ID,
                                                                                          TEST_BENEFICIARY_0_0);
        Assert.assertNotNull("returned response entity", beneficiaryResponse);
        Assert.assertTrue("Expires header gt 0", beneficiaryResponse.getHeaders().getExpires() > 0);

        final Beneficiary beneficiary = beneficiaryResponse.getBody();
    ...
    public void testAddBeneficiary() {
        final String beneficiariesUrl = "http://testhost:8080/rewards/rest/accounts/" + ACCOUNT0_ID + "/beneficiaries";

        final ResponseEntity<Object> response =
                controller.addBeneficiary(ACCOUNT0_ID, TEST_BENEFICIARY_0_0, beneficiariesUrl);

        Assert.assertEquals("response.locationHeader", beneficiariesUrl + '/' + TEST_BENEFICIARY_0_0,
                            response.getHeaders().getLocation().toString());
        Assert.assertEquals("response.statusCode", HttpStatus.CREATED, response.getStatusCode());
        Assert.assertFalse("no response.body", response.hasBody());

        verify(accountManager, times(1)).addAccountBeneficiary(ACCOUNT0_ID, TEST_BENEFICIARY_0_0);
        verifyNoMoreInteractions(accountManager);
    }
    ...
    public void testCreateBeneficiary() throws Exception {
        final String beneficiariesUrl =
                "http://testhost:8080/rewards/rest/accounts/" + ACCOUNT0_ID + "/beneficiaries" + '/' +
                TEST_BENEFICIARY_0_0;

        final ResponseEntity<Object> response =
                controller.createBeneficiary(ACCOUNT0_ID, TEST_BENEFICIARY_0_0, new URI(beneficiariesUrl));

        Assert.assertEquals("response.locationHeader", beneficiariesUrl,
                            response.getHeaders().getLocation().toString());
        Assert.assertEquals("response.statusCode", HttpStatus.CREATED, response.getStatusCode());
        Assert.assertFalse("no response.body", response.hasBody());
    ...
    /**
     * Test handle DataIntegrityViolationException
     */
    @Test
    public void testHandleAlreadyExists() {
        final DataIntegrityViolationException ex = new DataIntegrityViolationException("alredy exists");
        final String actualMsg = controller.handleAlreadyExists(ex);
    ...
    private Account createAccount0() {
        ...
        account0.setEntityId(ACCOUNT0_ID);
        return account0;
    }
